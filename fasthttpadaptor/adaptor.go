...
func NewFastHTTPHandler(h http.Handler) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		...
		w := netHTTPResponseWriter{w: ctx.Response.BodyWriter(), ctx: ctx}
		h.ServeHTTP(&w, r.WithContext(ctx))
		if w.hijacked {
			// request hijacked, skip fasthttp response
			ctx.Response.Reset()
			return
		}
...
func (w *netHTTPResponseWriter) Write(p []byte) (int, error) {
	if w.hijacked {
		log.Println("http: response.Write on hijacked connection")
...
func nopeHijacker(net.Conn) {}

func (w *netHTTPResponseWriter) Hijack() (net.Conn, *bufio.ReadWriter, error) {
	w.hijacked = true

	// previous body
	body := w.ctx.Response.Body()
	if len(body) != 0 {
		n, err := w.ctx.Conn().Write(body)
		if err != nil {
			return nil, nil, err
		}
		...
		w.ctx.Response.SetBody(nil)
	}

	// hijacker is not actually called here, it's called after fasthttp.Handler return
	// we need to call it to tell current conn is hijacked
	w.ctx.Hijack(nopeHijacker)
	// otherwise fasthttp will write a 200 default response to conn and close it.
	w.ctx.HijackSetNoResponse(true)

	return w.ctx.Conn(), &bufio.ReadWriter{
		Reader: bufio.NewReader(w.ctx.Conn()),
		Writer: bufio.NewWriter(w.ctx.Conn()),
	}, nil
}
